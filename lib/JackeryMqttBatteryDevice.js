...
module.exports = class JackeryMqttBatteryDevice extends JackeryMqttDevice {
  ...
  async onDeviceState(body) {
    ...
    if (body.wsig !== undefined && body.wsig !== null && this.hasCapability('alarm_wifi')) {
      const disconnected = Number(body.wsig) === 0;
      this.setCapabilityValue('alarm_wifi', disconnected).catch(this.error);
    }
